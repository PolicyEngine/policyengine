	...
	if (parameter.unit === "/1") {
		...
		for(let currency in CURRENCY_SYMBOLS) {
			if(parameter.unit === currency) {
				result = {
					formatter: (value, noPeriod) => `${value < 0 ? "- " : ""}${CURRENCY_SYMBOLS[currency]}${parseFloat(Number(Math.abs(Math.round(value)))).toLocaleString()}${period && !noPeriod && (parameter.quantityType === "flow") ? ("/" + period) : ""}`,
				}

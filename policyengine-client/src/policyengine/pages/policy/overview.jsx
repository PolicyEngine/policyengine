...
function generateStepFromParameter(parameter, editingReform, country, page) {
	...
	if ((parameter[targetKey] !== parameter[comparisonKey]) && (!hide | (page !== "population-impact"))) {
		...
		return <Step
			key={parameter.name}
			status="finish"
			title={<Divider><><>{parameter.label}</> <>{populationSimCheckbox}</></> <Button style={{ border: "none" }} onClick={() => country.updatePolicy(parameter.name, parameter[comparisonKey])} > <CloseCircleOutlined> </CloseCircleOutlined> </Button></Divider>}
